											...
											<td><c:choose>
													<c:when test="${userRole eq 'ROLE_ADMIN'}">
														...
														<a class="btn btn-view"
															href="<c:url value="/worksCreation/view?workId=${viewDetails.id}"/>"
													...
													<c:otherwise>
														<c:choose>
															<c:when
																test="${viewDetails.status eq 'WORK_ESTMASN_CMPLTD'}">
																<c:url
	...
	<script>
		...
		$('#submitBtn').on(
				...
				function(event) {
					...
					alertify.confirm('Create New Work',
							'Are you Sure, want to create new work! ',
							function() {
								$.blockUI({
									css : {
										border : 'none',
