...
public class ViewDetailsDaoImpl implements  ViewDetailsDao{
	...
	List<Object[]> details=sessionFactory.getCurrentSession().
			createQuery("select distinct(a.id),d.name,e.name,a.workDetails,a.workNo,f.departmentName,g.surveyNo,h.consultant_firm,j.name,a.id,a.workStatus,a.status from Works a\r\n" + 
	...
	List<Object[]> details=sessionFactory.getCurrentSession().
			...
			"left join ConsultantInfo h on a.id=h.work\r\n" + 
			"left join TenderingProcess i on a.id=i.work\r\n" + 
			"left join Agency j on  i.angencyName=j.id where a.isActive='true' order by a.id desc")
			.list();
